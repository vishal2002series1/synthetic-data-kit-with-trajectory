...
- `transformations_demo_60.jsonl` - 60 transformed queries (30× expansion from 2 seeds)
- `expanded_queries_30x.jsonl` - Complete 30× expansion examples
...
- `multi_iteration_test.jsonl` - Test trajectories with CALL→CALL→ANSWER patterns
...
- **Q&A Generation**: 1 document → 5-10 Q&A pairs
- **Persona Transformation**: 1 query → 5 variants (P1-P5)
- **Complexity Transformation**: 1 query → 3 variants (Q-, Q, Q+)
- **Tool Data Variants**: 1 query → 2 variants (correct/incorrect)
- **Total**: 1 seed query → 30 training variants (5 × 3 × 2)
...
- ✅ Generate coherent chain-of-thought reasoning
- ✅ Handle different user personas and complexity levels
