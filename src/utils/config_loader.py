"""
Configuration loader for Trajectory Synthetic Data Generator.
"""
...
class Config:
    ...
    def _parse_config(self):
        ...
        self.generation = GenerationConfig(
            target_qa_pairs=gen.get('target_qa_pairs', 50),
            expansion_factor=gen.get('expansion_factor', 30),
        ...
        if 'schema' in out:
            ...
            if 'fields' in schema_cfg:
                ...
                fields = OutputSchemaFields(
                    query=fields_cfg.get('query', 'Q'),
                    cot=fields_cfg.get('cot', 'COT'),
                    tools=fields_cfg.get('tools', 'Tool Set'),
                    decision=fields_cfg.get('decision', 'Decision')
                )
            else:
                fields = OutputSchemaFields()
